...
- Recompute Block 021 effective current state with explicit `from_scratch` immediately before write,
  temporarily refreshing both selected TrackInfo caches and the AlbumInfo caches consumed by album
  fallback.
- Validate exact stale state, scalar-string values, target shapes, and unique targets before mutation.
- Restore exact pre-attempt AlbumInfo cache state after validation; AlbumInfo is not a mutation target.
- Mutate only selected `TrackInfo.lyrics` and permanently invalidate only its `raw_data` and
  `item_data` caches.
